...
parser_error_t checkSpends(const parser_tx_t *txObj, uint32_t zip32_account, parser_context_t *builder_spends_ctx, parser_context_t *tx_spends_ctx, parser_context_t *indices_ctx) {
    if (txObj == NULL) {
...
zxerr_t crypto_check_masp(const parser_tx_t *txObj, uint32_t zip32_account) {
    ...
    parser_context_t spends_indices_ctx = {.buffer = txObj->transaction.sections.maspBuilder.metadata.spends_indices.ptr,
                                        ...
                                        .offset = 0, 
                                        .tx_obj = NULL};
    CHECK_PARSER_OK(checkSpends(txObj, zip32_account, &builder_spends_ctx, &tx_spends_ctx, &spends_indices_ctx));
